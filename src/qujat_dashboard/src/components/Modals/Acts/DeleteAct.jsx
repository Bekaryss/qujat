...
import { deleteAct } from "../../../redux/slices/actsSlice";
import close from "../../../assets/images/close.svg";

const Delete = ({ deleteItem, handleCloseDeleteModal }) => {
  const { id } = useSelector((state) => state.acts);
  const dispatch = useDispatch();

  const handleDelete = async (event) => {
    event.preventDefault(); // Предотвращение стандартного действия формы
    const response = await dispatch(deleteAct(id));
    ...
    // } else {
    //   console.error(
    //     `Failed to delete category. Status: ${deleteResponse.status}`
    //   );
    // }
  };
